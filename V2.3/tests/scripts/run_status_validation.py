...
class StatusValidationRunner:
    ...
    def run_integration_tests(self) -> Dict[str, Any]:
        ...
        try:
            # Ensure UTF-8 encoding for child process and decoding in parent on Windows
            env = os.environ.copy()
            ...
            result = subprocess.run(
                ...
                capture_output=True, 
                text=True,
                encoding='utf-8',   # decode stdout/stderr as UTF-8 to avoid GBK decode errors
                errors='replace',   # replace undecodable chars instead of raising
    ...
    def generate_combined_report(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if result['type'] == 'integration_tests':
                ...
                # Robustly parse test count from stdout
                test_count = 0
                ...
                if test_count:
                    ...
                    # Fallback: if success but count not found, count as 1
                    if result['success']:
                        total_tests += 1
                        passed_tests += 1
                ...
                if isinstance(detailed, dict) and 'run' in detailed:
                    run_stats = detailed['run'].get('stats', {})
                    assertions = run_stats.get('assertions', {})
                    total = assertions.get('total') or 0
                    failed = assertions.get('failed') or 0
                    pending = assertions.get('pending') or 0
                    passed = assertions.get('passed')
                    if passed is None:
                        # newman JSON often lacks 'passed', derive it
